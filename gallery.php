                ...
                <header class="page-heading clearfix">
                    ...
                    <div class="breadcrumbs pull-right">
                        <ul class="breadcrumbs-list">
                            ...
                            <li class="breadcrumbs-label">Gallery</li>
                        </ul>
                    </div><!--//breadcrumbs-->
                </header>
                <div class="page-content">
                    <div class="page-row">
                        <p>Berbagai kegiatan untuk menunjang kegiatan belajar mengajar baik di SD ataupun di TK bertujuan untuk membentuk kepribadian setiap anak didik kami menjadi pribadi-pribadi yang berakhlak mulia, melatih mental serta menummbuhkan rasa kekeluargaan.</p>

                    </div>
                    <div class="row page-row">
                        <?php
                        $list_gallery = mysqli_query($link, "select * from kegiatan");
                        while($data = mysqli_fetch_array($list_gallery)){

                            $gallery = mysqli_query($link, "select * from gallery WHERE id_kegiatan='$data[id_kegiatan]' limit 1");
                            while($row = mysqli_fetch_array($gallery)){
                                $direct = sha1(sha1($row[1]).sha1('Sq1-Inj3Ct10N'));
                                ?>
                                <div class="col-md-4 col-sm-4 col-xs-12 text-center">
                                    <div class="album-cover">
                                        <a href="gallery-album.php?list=<?= abs((int) $row[1]) ?>&page=<?= $direct ?>"><img class="img-responsive" src="dashboard/img/gallery/<?= $row['foto'] ?>" alt="" /></a>
                                        <div class="desc">
                                            <h4><small><a href="gallery-album.php?list=<?= abs((int) $row[1]) ?>&page=<?= $direct ?>"><?= $data['judul'] ?></a></small></h4>
                                            <p>Pada saat kegiatan <?= $data['judul'] ?> di Yayasan Aksara Buana tahun <?= $data['tahun'] ?>.</p>
                                        </div>
                                    </div>
                                </div>
                                <?php
